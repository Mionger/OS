...
void Machine::InitPageDirectory()
{
	...
	/* 填写200#页表的第0项，使线性地址0-4M映射到物理内存0-4M */
...
{
	/* 重新映射线性地址0M-8M，用新页表代替原来的系统内核的页表 */
	...
	for ( unsigned int j = 0; j < USER_PAGE_TABLE_CNT; j++, idx++ )
	{
		...
		for ( unsigned int i = 0; i < PageTable::ENTRY_CNT_PER_PAGETABLE; i++ )
		{
			...
			/* 
			 * 初始时设置用户页表，重新映射线性地址0M-8M到物理内存的4M-8M，
			 * 其实当前内核初始化阶段并不使用这段内存，也不会向4M-8M物理内存
			 * 写入任何数据。因此PageBaseAddress全部设为0x00000一样OK!
			 */
			pUserPageTable[j].m_Entrys[i].m_PageBaseAddress = 0x00000 /*0x400*/ + i + j * 1024;
		}
	}
